...
- **本地图片存储**：在设置里开启「使用本地 IndexedDB 存储图片」后，刷新页面也能恢复新粘贴图片的预览

### 2. 选区 AI 写作

选中左侧编辑器里的一段文字，右上角会出现一个小机器人。悬浮或触摸它，可以选择：

- **格式化**：把复制进来挤成一段的素材整理成公众号文章结构，自动拆出标题、段落、列表和表格
- **扩写**：在保留原意的基础上补充信息，让内容更完整
- **改写**：优化表达、语气和可读性，适合发布前润色

> 打开「设置 → AI Writing」填写 \`BASE_URL\`、\`API_KEY\` 和 \`MODEL\` 即可使用。配置只保存在你的浏览器本地。AI 正在处理时，小机器人会显示渐显渐隐的呼吸灯状态。
...
- 选中一段文字，悬浮小机器人，体验 AI 格式化/扩写/改写
- 直接粘贴截图或图片，并在设置中开启本地图片存储
- 点击设备图标切换手机/平板/桌面预览
- 清空以上内容，开始你的创作
`;
