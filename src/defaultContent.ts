...
- **本地图片缓存**：在设置里开启「使用本地DB缓存图片」后，刷新页面也能恢复粘贴图片的预览

### 2. AI 写作助手

打开「设置 → AI Writing」配置AI相关信息后，系统会自动检测模型是否可用。检测通过后，左侧编辑器右上角会默认显示小机器人：有选区时处理选区，没有选区时处理整篇原文。

- **格式化**：整理结构与排版，把复制进来的素材变成公众号文章结构
- **扩写**：补充细节，让内容更完整
- **改写**：优化表达和语气，适合发布前润色

> API Key 只保存在你的浏览器本地。默认开启「使用 [RELAYX](https://github.com/xCss/relayx) 转发AI请求」，可减少浏览器直连模型服务时遇到的跨域限制。
