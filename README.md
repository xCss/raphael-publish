...
开启「使用本地DB缓存图片」后，新粘贴的图片会保存在浏览器本地，刷新页面后仍可恢复预览。关闭缓存时可选择是否保留 `raphael-image://` 引用，方便后续手动恢复或清理正文。
...
API Key 仅保存在浏览器本地。默认开启「使用 [RELAYX](https://github.com/xCss/relayx) 转发AI请求」，用于减少浏览器直连模型服务时遇到的跨域限制；AI 操作进行中时，机器人会显示呼吸灯加载状态。
...
- **自动更新检测**：在线状态下每 30 秒检查一次 Service Worker 更新，开发环境默认不启用 PWA，避免缓存干扰调试。
