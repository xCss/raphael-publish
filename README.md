...
开启「使用本地 IndexedDB 存储图片」后，新粘贴的图片会保存在浏览器本地，刷新页面后仍可恢复预览。

### 选区 AI 写作

在编辑器中选中一段文字后，右上角会出现 AI 小机器人。悬浮或触摸机器人即可选择：

- **格式化**：将粘贴后挤成一段的内容整理成公众号友好的 Markdown 结构。
- **扩写**：在保留原意的前提下补充细节，让段落更完整。
- **改写**：优化表达、语气和可读性，适合发布前润色。

AI 配置在「设置 → AI Writing」中填写 `BASE_URL`、`API_KEY` 和 `MODEL`。配置仅保存在浏览器本地；AI 操作进行中时，机器人会显示呼吸灯加载状态。
